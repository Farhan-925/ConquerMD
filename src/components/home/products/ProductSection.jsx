...
export default function ProductSection({ collection }) {
  ...
  const handleAddToCart = (product) => {
    // Safely convert string priceTag "$49.99" to number 49.99 if numeric price is missing
    // Safely convert string priceTag "$49.99" to number 49.99 if numeric price is missing
  const numericPrice =
  (product.price ?? parseFloat(product.priceTag?.replace(/[^0-9.]/g, ""))) || 0;
...
function ProductCard({ product, theme, onAddToCart }) {
  // Determine displayed price on top tag
  const displayPrice = product.priceTag || (product.price ? `$${product.price.toFixed(2)}` : null);
  ...
  return (
    <div className="rounded-[24px] overflow-hidden flex flex-col h-full shadow-sm">
      ...
      {/* BOTTOM SECTION */}
      <div className={`${theme.cardBgBottom} p-6 flex flex-col justify-between gap-4 flex-1`}>
        <div>
        ...
        <div className="flex items-center justify-between gap-2 pt-2">
          {/* Add To Cart Button (Price Removed) */}
          <button
            type="button"
            onClick={onAddToCart}
            className="flex-1 flex items-center justify-center gap-2 bg-emerald-800 text-white py-3 px-4 rounded-full text-xs font-bold uppercase tracking-wider hover:bg-emerald-900 transition-colors cursor-pointer"
          >
